...
class Rss_Image_Feed {
	...
	function get_feed_image() {
		...
		if ($rif_image_info) :
			...
			$rif_image_width = (!self::$options['responsive']) ? '" width="'.$rif_width.'"'.$rif_height : '" style="max-width: '.$rif_width.'px; height: auto;"';
		
			$eol = "\n";
			$tab = "\t";
		
			$rif_img_tag = '<a href="'.get_permalink().'"><img title="'.$rif_image_title.'" src="'.$rif_thumb.'" alt="'.$rif_image_alt.$rif_image_width.' /></a>';
